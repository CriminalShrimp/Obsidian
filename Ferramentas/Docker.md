...
Para automação de implantação, escalonamento e gerenciamento de aplicações em contêineres. Quando se tem apenas um contêiner para gerenciar é fácil, mas em um cenário onde existam vários se tornar muito difícil cuidar de todos, é ai que o **Kubernetes** ou **K8s** (do grego piloto ou timoneiro) entra, ele é uma plataforma de código aberto que orquestra os contêineres, em outras palavras, ele realiza mudanças e melhorias em suas aplicações, se algo da errado durante isso ele mesmo volta a versão e reinicia contêineres que falharam, entre outras coisas que facilitam a vida de quem gerência contêineres. Ele realiza esse gerenciamento através de <span style="color:rgb(255, 255, 0)"><b>Pods</b></span>, e nesse pode é onde temos o contêiner, sendo possível colocar mais que um contêiner em um Pod (mas é um uso especifico). A criação de Pods é feita em YAML assim como o DockFile, mas não criamos isso de formar manual, ao invés disso usamos os <span style="color:rgb(255, 255, 0)">Workloads</span> (ou **Cargas de Trabalho**), que é uma aplicação do próprio Kubernetes, e podemos fazer diversas coisas usando os recursos dessa aplicação como `replicaSet`, que coloca um numero de replicas da aplicação, ou o `deployment` onde é possível definir diversos parâmetros para os pods. Os contêineres rodam normalmente em uma maquina/sistema operacional, já os pods se localizam em um servidor ou maquina virtual, e esses são chamados de <span style="color:rgb(255, 255, 0)">Node</span> (Nó), deve haver um nome único para cada Node. E é no <span style="color:rgb(255, 255, 0)">Cluster</span> que toda a magia acontece, nele podemos ter diversos Nodes, e também temos o <span style="color:rgb(255, 255, 0)">Control Plane</span> (**Plano de Controle**), que seria uma espécie de servidor **master** (principal), onde se encontram os componentes essências do Kubernetes.

É fácil se perder com tantos nomes, então aqui tem uma imagem para visualizar sobre o que estamos vendo e um mini resumo do básico (sim, isso é básico ainda) do que precisamos saber:

- **Pods**: São onde colocamos nosso contêiner para podermos gerenciarmos ele de forma mas fácil. Usamos recurso do **Workloads** para fazer esse gerenciamento.
	- **Node**: Aqui é onde colocamos os nossos Pods, pense no Pod como um sistema operacional e o Node como a maquina em que instalamos.
		- **Cluster**: É onde ficam todos os Nodes, e também os recursos necessários para o Kubernetes funcionar, que ficam no **Control Plane**.

<figure style="text-align: center;">
  <img src="Kubenetes-Cluster.png" style="margin: 0 auto;">
